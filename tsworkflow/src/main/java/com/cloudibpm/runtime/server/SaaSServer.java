...
public class SaaSServer implements java.io.Serializable {
	...
	public final Map<String, Callable<WfProcessInstance>> activePThreads = Collections.synchronizedMap(new HashMap<>());
	...
	/**
	 * Power off this server. That means this server will stop accept all services
	 * on this server and then
	 */
...
//        }
	...
	// APIs
	public AdminSearchResult[] search(String oid, String cond1, String cond2, String cond3, String cond4)
			throws Exception {
		if (pEnginPool != null) {
	...
	// search app APIs
	public AdminSearchResult[] searchApp(String pid, String cond1, String cond2, String cond3, String cond4)
			throws Exception {
		if (pEnginPool != null) {
	...
	public AdminSearchResult[] getAllInstances(String oid, String cond1, String cond2, String cond3, String cond4)
			throws Exception {
		if (pEnginPool != null) {
			List<AdminSearchResult> list = new ArrayList<AdminSearchResult>();
			synchronized (activePThreads) {
	...
	private boolean search(WfProcessInstance inst, String cond1, String cond2, String cond3, String cond4)
			throws Exception {
		if (cond1 == null)
			cond1 = "";
		if (cond2 == null)
		...
		if (!cond1.trim().equals("")) {
			if (cond1.indexOf(" ") > 0) {
				...
				for (int i = 0; i < conds.length; i++) {
					if (inst.getName().contains(conds[i]) || inst.getLaunchUser().contains(conds[i])
							|| inst.getIdNumber().contains(conds[i]) || inst.getMobileNumber().contains(conds[i])
				...
				if (inst.getName().contains(cond1) || inst.getLaunchUser().contains(cond1)
						|| inst.getIdNumber().contains(cond1) || inst.getMobileNumber().contains(cond1)
		...
		if (cond3.equals("") && cond4.equals("")) {
			...
			Date fromDate = DateUtility.parseDatetime(cond3);
			Date toDate = DateUtility.parseDatetime(cond4);
			if (fromDate.getTime() <= inst.getLaunchTime() && toDate.getTime() >= inst.getLaunchTime()) {
				f2 = true;
				f3 = true;
			}
		}
	...
	/**
	 * @param acceptable the acceptable to set
	 */
	...
	/**
	 *
	 * @param pid         process instance object
	 ...
	 * @throws Exception
	 */
	public WfProcessInstance launch(String pid, String userid, String ipv4, String ipv6, String longitude,
			String latitude, String device, Map<String, Object> paramvalues) throws Exception {
		...
		if (pEnginPool != null) {
			...
			if (pid.length() == 16) { // process definition ID
				...
				ProcessInstanceInitializer.getInstance().initialize(instance, ipv4, ipv6, "", longitude, latitude,
						device, paramvalues);
			} else if (pid.length() == 32) { // process instance ID
				instance = ProcessInstanceLoader.getInstance().loadCompletedInstance(pid);
				// initializing （将初始化数据写入流程变量）
				ProcessInstanceInitializer.getInstance().reinitialize(instance, ipv4, ipv6, "", longitude, latitude,
						device, paramvalues);
			}
	...
	public WorkitemInfoDescriptor[] fetchManualTaskInstances(String userId, String userfullname, String staffids,
			String cond, String qtype) throws Exception {
...
//        }
	 ...
	 *
	 * @param ti           ManualTaskInstance
	 * @param inst         WfProcessInstance
	 * @param userfullname
	 * @param priority     String, 0: low priority(normal); 1: medium
	 *                     priority(important); 2: high priority(urgent);
	 * @return
	 */

	private WorkitemInfoDescriptor getWorkitemInfoDescriptor(ManualTaskInstance ti, WfProcessInstance inst,
			String userId, String userfullname, String priority) {
	...
	public WorkitemInfoDescriptor fetchManualTaskInstanceforMWf(String pInstId, String taskInstId, String userId,
			String userIp, int priority, String ufullname) throws Exception {
		synchronized (activePThreads) {
			PEngine engine = (PEngine) activePThreads.get(pInstId);
			WorkitemInfoDescriptor w = new WorkitemInfoDescriptor();
			if (engine != null) {
				ManualTaskInstance ti = engine.fetchManualTaskInstanceforMWf(taskInstId, userId, userIp, priority,
						ufullname);
				// 解析表单（Form）并预装数据到表单。
				if (ti != null) {
					// w.setFormContent(FormDataLoader.parseJSONtoLoadData(ti.getFormContent(),
					// engine.getInstance()));
					w.setFormContent(
							FormDataLoader.parseJSON2LoadData((JSONObject) ti.getFormContent(), engine.getInstance()));
				}
			}
			return w;
		}
	...
	public WorkitemInfoDescriptor fetchManualTaskInstanceforSWf(String pInstId, String tid, String userId,
			String userIp, String fullname) throws Exception {
		synchronized (activePThreads) {
			PEngine engine = (PEngine) activePThreads.get(pInstId);
			if (engine != null) {
	...
	public String saveManualTaskInstance(String pInstId, String taskInstId, String userId, Map<String, Object> list)
			throws Exception {
		synchronized (activePThreads) {
			PEngine engine = (PEngine) activePThreads.get(pInstId);
			if (engine != null) {
	...
	public String returnManualTaskInstance(String pInstId, String taskInstId, String userId) throws Exception {
		synchronized (activePThreads) {
			PEngine engine = (PEngine) activePThreads.get(pInstId);
			if (engine != null) {
	...
	public String submitManualTaskInstance(String pInstId, String taskInstId, String userId, String userIp,
	...
	public String addInvitation(String pid, String tid, String copyto) throws Exception {
		PEngine engine = (PEngine) activePThreads.get(pid);
		if (engine != null) {
			ManualTaskInstance ti = engine.getManualTaskInstanceforMWf(tid);
			synchronized (ti) {
				ti.addInvitation(copyto);
			}
		}
		return "1";
	}
