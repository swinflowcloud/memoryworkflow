...
//		this.radiosInput2.addEventListener("click", this, false);
		...
		radiosDIV1.appendChild(radiosText2);
		...
		this.radiosInput5.checked = true;
	...
	CreateModelDialog.prototype.doKeydown = function(evt) {
		...
		if (key == 13) {
			if (evt.target.value != "") {
				//var c = $('input[type="radio"][name="optionsRadios"]:checked').val();
				this.createNewObject(2, evt.target.value);
			} else {
	...
	CreateModelDialog.prototype.doClick = function(evt) {
		if (evt.target == this.saveButton) {
			...
			this.createNewObject(2, name1, ptype);
