...
(function($, window, document, undefined) {
	...
	SearchConditionPanel.prototype.doClick = function(evt) {
		if (evt.target == this.firstPageHButton
			...
			this.loading(this.options.pid, 1, 30, this.searchInput.value,
					this.statusSelect.value, this.fromDateInput.value,
	...
	SearchConditionPanel.prototype.loadData = function(obj) {
		...
		if (objs != null && objs.length > 0) {
			for (var i = 0; i < objs.length; i++) {
				...
				this.createCell(8, objs[i].suspensionTime == -1 ? "" : Utils
						.parseDateTime(objs[i].suspensionTime), row);
				this.createCell(9, objs[i].updateTime == -1 ? "" : Utils
						.parseDateTime(objs[i].updateTime), row);
